	...
	<div class="container">
		<form method="post" action="usersServlet">
				...
				<c:forEach items="${requestScope.allTasks}" var="currenttask">
					<tr>
						<td><input required type="radio" name="id"
							value="${currenttask.taskID}"></td>
						<td>${currenttask.taskID}</td>
						<td>${currenttask.taskInfo}</td>
						<td>${currenttask.startDate}</td>
			...
			<input type="submit" value="complete" name="doThisToUser"
				...
				type="button" name="doThisToUser" value="exit" class="btn btn-info"></a>
